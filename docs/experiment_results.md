...
单次标定误差和扰动实验不能反映参数包络内的最坏样本。使用固定种子执行可重复扫参：

```bash
./scripts/run_robustness_sweep.py --trials 12 --seed 20260923
```

每次试验随机化以下参数，范围在运行前固定：

- 右臂基座位置误差：x/y 为 +/-4 mm，z 为 +/-3 mm；
- 右臂基座姿态误差：roll/pitch 为 +/-0.2 deg，yaw 为 +/-0.5 deg；
- 外力大小为 2--6 N，方向在球面上随机；
- 外力矩三个分量分别位于 +/-0.25 N m；
- 扰动开始时刻为 8.5--12.0 s，持续 0.6--1.2 s。

脚本复用 `slot_avoid` 的任务级验收规则，并生成每个样本的实际参数、检查项、原始日志路径、成功率和最坏指标。固定 seed 便于提交前回归；更换 seed 或增加 trial 数用于扩大覆盖。该方法仍是有限参数包络中的确定性仿真采样，不应解释为真实系统的概率安全保证。

固定 seed `20260923` 的 12 次回归结果：

| 指标 | 结果 |
|---|---:|
| 通过率 | 12/12（100%） |
| 最小 `object~barrier` 距离 | 10.000 mm |
| 最小 `object~slot_frame` 距离 | 0.893 mm |
| 最大终态位置误差 | 0.501 mm |
| 最大终态姿态误差 | 0.188 deg |
| 最大单次运行 controller P99 | 292.3 us |
| 力矩饱和步数 | 0 |
